...
async function filterOrdersDFOptions(df, categories, extra_fields_to_pull=[], names_of_extra_fields=[])  {
...
export async function getRealizedProfitOptions(buyOrders, sellOrders){
    ...
    let categories = ['chain_symbol','quantity', 'legs', 'state', 'premium', 'direction', 'processed_premium'];
    ...
    for(const row of await sellDF){
        ...
        let price = parseFloat(getFieldFromOptionsRowDF(row, 'processed_premium', false)) / quantity;
    ...
    for(const row of await buyDF){
        ...
        if(!(ticker in weighted_avg) || quantity_dict[ticker] === 0) { 
            ...
            let expiration_date = new Date(await api.getFieldFromInstrumentOption(HEADER, instrument, 'expiration_date'));
            if (expiration_date < new Date()){ // option has expired
                // console.log('expired option:', ticker, price);
                if (!(ticker in weighted_avg)){ // an option of this ticker has never been sold
                    weighted_avg[ticker] = -price * quantity;
                    quantity_dict[ticker] = 0;
                    instruments[ticker] = instrument;
                }
                else { // an option of this ticker has been sold, but this specific option expired
                    weighted_avg[ticker] -= price * quantity;
                    quantity_dict[ticker] += quantity;
                }
...
async function filterOrdersDF(df, categories, extra_fields_to_pull=[], names_of_extra_fields=[])  {
    if(df.columns.size === 0){
        return df;
    }
...
export async function getRealizedProfit(buyOrders, sellOrders){
    ...
    buyDF = await filterOrdersDF(buyDF, categories, extra_fields_to_pull, names_of_extra_fields);
    sellDF = await filterOrdersDF(sellDF, categories, extra_fields_to_pull, names_of_extra_fields);
