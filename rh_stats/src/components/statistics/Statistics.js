...
const equity_columns_to_display = [
  "Name",
  "Average Cost",
  "Equity",
  "Dividend",
...
const options_df_columns = [
  ...
  "realized profit",
  "unrealized profit",
  "percent unrealized profit",
];
const options_columns_to_display = ["Name", "Average Cost", "Realized Return"];
const all_options_fields = [
  ...options_columns_to_display,
  "Tradability",
  "Quantity",
  "Unrealized Percent Return",
];
...
export const Statistics = (props) => {
  ...
  const updateOptionsData = async () => {
    ...
    if (optionsHistoryDF)
      optionsPositionsDF = optionsPositionsDF.get([
        "symbol",
        "average_open_price",
        "quantity",
        "legs",
    ...
    let realProfit = await analysis.getRealizedProfitOptions(
      buyOrders,
      sellOrders
    );
    ...
    if (profitDF.length) {
      ...
      newMerged = newMerged.set(
        "realized profit",
        utils.zeroesArray(merged.length)
      );
    }
    ...
    const getSortedRows = (df, df_columns) => {
      ...
      for (const row of df) {
        let dataRow = df_columns.map((col) => {
          let val = row.get(col);
          if (parseFloat(val)) val = parseFloat(row.get(col));
          return val;
        });
  ...
  const sortColumns = (dataRows, category) => {
    ...
    dataRows.sort((a, b) => {
      ...
      if (!a[index]) return Number.MAX_SAFE_INTEGER;

      if (!b[index]) return Number.MIN_SAFE_INTEGER;
  ...
  function populateEquitySpecRender() {
    let symbol_obj_idx = findIdxByDFColumnName(equity_specs, "symbol"),
      ...
      unrealized_obj_idx = findIdxByDFColumnName(
        equity_specs,
        "unrealized profit"
      ),
      ...
      averageCost_obj_idx = findIdxByDFColumnName(
        equity_specs,
        "average_buy_price"
      ),
      earningPotential_obj_idx = findIdxByDFColumnName(
        equity_specs,
        "earning potential"
      );
    ...
    const renderPriceButton = (symbol, tradability, currentPrice) => {
      if (tradability !== "tradable") return null;

      return (
        <button
          onClick={() => window.open("http://robinhood.com/stocks/" + symbol)}
          target="_blank"
    ...
    if (equity_specs[realized_obj_idx])
      equity_specs[realized_obj_idx].render = () => {
        ...
        return (
          <div
            key="realized profit"
            className={`cell text ${columnClass} ${realizedClass}`}
          >
            {realReturnString}
          </div>
        );
      ...
      equity_specs[unrealized_obj_idx].render = () => {
        ...
        return (
          <div
            ...
            className={`cell text ${columnClass} ${unrealizedClass}`}
          >
            {unrealReturnString}
          </div>
        );
    ...
    if (equity_specs[equity_obj_idx])
      equity_specs[equity_obj_idx].render = () => {
        return (
          <div key="equity" className={`cell text ${columnClass}`}>
            {utils.beautifyPrice(equity_specs[equity_obj_idx].data)}
          </div>
        );
      };
    ...
    if (equity_specs[dividend_obj_idx])
      equity_specs[dividend_obj_idx].render = () => {
        return (
          <div key="dividend" className={`cell text ${columnClass}`}>
            {utils.beautifyPrice(equity_specs[dividend_obj_idx].data)}
          </div>
        );
      };
    ...
    if (equity_specs[averageCost_obj_idx])
      equity_specs[averageCost_obj_idx].render = () => {
        return (
          <div key="average cost" className={`cell text ${columnClass}`}>
            {utils.beautifyPrice(equity_specs[averageCost_obj_idx].data)}
          </div>
        );
      };
    ...
    if (equity_specs[earningPotential_obj_idx])
      equity_specs[earningPotential_obj_idx].render = () => {
        return (
          <div key="earnings potential" className={`cell text ${columnClass}`}>
            -
          </div>
        );
      };
  ...
  function populateOptionsSpecRender() {
    ...
    let symbol_obj_idx = findIdxByDFColumnName(options_specs, "symbol"),
      ...
      realized_obj_idx = findIdxByDFColumnName(
        options_specs,
        "realized profit"
      ),
      unrealized_obj_idx = findIdxByDFColumnName(
      ...
      unrealized_percent_obj_idx = findIdxByDFColumnName(
        options_specs,
        "percent unrealized profit"
      ),
    ...
    if (options_specs[averageCost_obj_idx])
      options_specs[averageCost_obj_idx].render = () => {
        return (
          <div key="average cost" className={`cell text ${columnClass}`}>
            {utils.beautifyPrice(options_specs[averageCost_obj_idx].data)}
          </div>
        );
      };
      ...
      <div className="row">
        {columns_to_display.map((elem, idx) => {
          return (
            <div
              key={idx}
              className={`cell text row-header ${columnClass} category`}
              onClick={() => sortDataByCategory(specs, elem)}
            >
              <div
                className="cell text row-header history-column-btn"
                key={idx}
              >
                {elem}
              </div>
    ...
    return optionsHistory.map((dataRow) => {
      for (let i = 0; i < options_specs.length; i++) {
        ...
        if (obj.df_column_name) {
          options_specs[i].data =
            dataRow[options_df_columns.indexOf(obj.df_column_name)];
        }
      }

      populateOptionsSpecRender();
      let key =
        options_specs[findIdxByDFColumnName(options_specs, "symbol")].data;
      return (
        <div key={key}>
          <div className="row">
  ...
  function renderEquityHistory() {
    ...
    return equityHistory.map((dataRow) => {
      for (let i = 0; i < equity_specs.length; i++) {
        ...
        if (obj.df_column_name) {
          equity_specs[i].data =
            dataRow[equity_df_columns.indexOf(obj.df_column_name)];
        }
      }
      ...
      return (
        <div
          key={equity_specs[findIdxByDFColumnName(equity_specs, "symbol")].data}
        >
          <div className="row">{equity_specs.map((obj) => obj.render())}</div>
          <hr />
        </div>
      );
    });
  ...
  function renderTable() {
    if (activeCategory === "equities") return renderEquityTable();
    else if (activeCategory === "options") return renderOptionsTable();
  }

  if (!loggedIn) return <Redirect to="/login" push={true} />;
